...
let
  composer = stdenv.mkDerivation {
    ...
    src = fetchurl {
      url =
        "https://github.com/composer/composer/releases/download/1.9.0/composer.phar";
      sha256 =
  ...
  buildPackage = { name, src, packages ? { }, devPackages ? { }
    ...
    in stdenv.mkDerivation ({
      ...
      installPhase = ''
        ...
        composer dump-autoload --optimize --no-scripts -vvv ${
          stdenv.lib.optionalString noDev "--no-dev"
        }

        # Run the install step as a validation to confirm that everything works out as expected
        composer install --optimize-autoloader --no-scripts -vvv ${
          stdenv.lib.optionalString noDev "--no-dev --no-scripts"
        }
